...
#### Nearest Neighbor Classification with Cross-Validation

Returns class predictions given test indices and k.
...
Returns class predictions given test indices and ntree.
...
#### Visualization with MDS 2D (on instance level)
...
#### Visualization with MDS 3D (on instance level)
...
Not possible to observe clusters visually.


### Representation Approaches for Bags


representation approach 1 = Applying k-means clustering and representing each bag with fraction of instances in each cluster to observe instance frequencies in regions of dataspace.(Şeyma Küçükaşcı & Gökçe Baydoğan, 2018)


representation approach  2 = Representing with mean distance of instances to each cluster. Instance-cluster distance is computed as the average distance between the related instance and observations in that cluster. Bag representation is obtained as the mean of distances to each cluster. 
...
iter.max is set to 25 in Şeyma Küçükaşcı and Gökçe Baydoğan and therefore it is also set to 25 in this work.

nstarts is set to 5 in the original application tuned with a list of 5 and 10 tried in this work.

(16,20,24,30,34,40) will be tried for number of centers.

### NN Classification Parameters

(1,3,5,7,10) will be tried as k.

### Random Forest Classification Parameters

(500,700,800) will be tried as ntree.
...
The algorithm worked with 0.81 accuracy independent of the parameters. 


### References


Şeyma Küçükaşcı, E., & Gökçe Baydoğan, M. (2018). Bag encoding strategies in multiple instance learning problems. Information Sciences, 467, 559–578. https://doi.org/10.1016/j.ins.2018.08.020
